...
vOutputChar(const char cChar, const TickType_t xTicksToWait) {}
...
static void strbuf_init(struct SStringBuf *apStr, char *apBuf,
                        const char *apMaxStr) {
  apStr->str = apBuf;
  apStr->orgStr = apBuf;
...
static BaseType_t printIp(struct SStringBuf *apBuf, unsigned i) {
  ...
  sprintf(print_buf, "%u.%u.%u.%u", i >> 24, (i >> 16) & 0xff, (i >> 8) & 0xff,
          i & 0xff);
  apBuf->flags.isNumber = pdTRUE; /* Parameter for prints */
...
static void tiny_print(struct SStringBuf *apBuf, const char *format,
                       va_list args) {
  char scr[2];
...
const char *mkSize(unsigned long long aSize, char *apBuf, int aLen) {
  ...
  sb = aSize;
  if (gb) {
    snprintf(apBuf, aLen, "%u.%02u GB", (unsigned)gb,
             (unsigned)((100 * mb) / 1024ul));
  } else if (mb) {
    snprintf(apBuf, aLen, "%u.%02u MB", (unsigned)mb,
             (unsigned)((100 * kb) / 1024ul));
  } else if (kb != 0ul) {
    snprintf(apBuf, aLen, "%u.%02u KB", (unsigned)kb,
