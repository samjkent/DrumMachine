...
uint8_t ledMapping[25] = {2,  9, 16, 0, 6, 13, 20, 4,  10, 17, 1,  8, 14,
                          21, 5, 12, 3, 7, 10, 11, 12, 13, 13, 15, 2};
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) { mode = (mode + 1) % 3; }
...
void check_inputs(void *p) {
  ...
  while (1) {
    ...
    switch(mode)
    {
      ...
      {
        // Update sequencer
        sequencer_set_pattern(sequencer_channel,
    (sequencer[sequencer_channel].note_on ^= 1 << key_pressed));
        // Update sequencer LEDs
        for(int i = 0; i < 16; i++){
            // Clear red note data
...
int main(void) {
  ...
  mcp23017_iodir(&hmcp, MCP23017_PORTB,
                 MCP23017_IODIR_ALL_OUTPUT | MCP23017_IODIR_IO5_INPUT |
                     MCP23017_IODIR_IO6_INPUT | MCP23017_IODIR_IO7_INPUT);
